...
export async function GET() {
  try {
    ...
    // 2. Fetch recently purchased nodes
    const recentNodePurchases = await prisma.alphaNode.findMany({
      where: {
        lastPurchaseTime: {
          gte: thirtyMinutesAgo,
        },
    ...
    const activities = [
      ...
      ...recentNodePurchases.map((node) => ({
        id: node.id,
        type: 'node_purchase',
        title: `Purchased ${node.name}`,
        description: `Quality: ${node.qualityScore}% | Latency: ${node.latencyMs}ms`,
        value: node.price ?? 0,
        isPositive: true,
