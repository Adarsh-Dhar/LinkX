...
export function usePortfolioSync() {
  ...
  const fetchServerBalances = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        return {
          wxtzBalance: Number(data.wxtzBalance || 0),
          usdcBalance: Number(data.usdcBalance || 0),
        }
      }
    } catch (error) {
      console.warn("Could not fetch server wallet balances:", error)
    }
    return null
  }, [])

  // Fetch current WXTZ/USDC price
  const fetchWxtzPrice = useCallback(async (): Promise<number> => {
    try {
      // Try to get price from price endpoint
      const res = await fetch("/api/market/price/WXTZ-USDC", {
        cache: "no-store",
      })
      if (res.ok) {
        const data = await res.json()
      ...
      console.warn("Could not fetch WXTZ price:", error)
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.length > 0) {
          ...
          if (latest.wxtzBalance > 0 && latest.usdcBalance > 0) {
            return latest.usdcBalance / latest.wxtzBalance
          }
        }
      }
    } catch (error) {
  ...
  const savePortfolioSnapshot = useCallback(
    async (wxtzBalance: number, usdcBalance: number, wxtzPrice: number) => {
      try {
  ...
  const syncPortfolio = useCallback(async (): Promise<PortfolioData | null> => {
    try {
      ...
      const walletWxtz = wallet.balance ? parseFloat(wallet.balance) : 0
      const walletUsdc = wallet.usdcBalance ? parseFloat(wallet.usdcBalance) : 0

      if (wallet.isConnected && (walletWxtz > 0 || walletUsdc > 0)) {
        wxtzBalance = walletWxtz
        usdcBalance = walletUsdc
        hasReliableBalances = true
      } else {
        const serverBalances = await fetchServerBalances()
        if (serverBalances) {
          wxtzBalance = serverBalances.wxtzBalance
      ...
      const wxtzPrice = await fetchWxtzPrice()
      if (!Number.isFinite(wxtzPrice) || wxtzPrice <= 0) {
        return null
      }

      // Prevent transient zero spikes from failed balance reads
      if (wxtzBalance === 0 && usdcBalance === 0) {
        return null
      }

      const totalValueUsd = wxtzBalance * wxtzPrice + usdcBalance

      // Save snapshot to database
      await savePortfolioSnapshot(wxtzBalance, usdcBalance, wxtzPrice)
  ...
  }, [wallet, fetchWxtzPrice, savePortfolioSnapshot, fetchServerBalances])
